<div class="row my-3">
    <div class="container question-container">
        @foreach(@$post->quizQuestions as $key => $quizQuestion)
            {{--            quiz-answered--}}
            <div class="personality-quiz-question " id="quiz_question_{{ $quizQuestion->id }}" data-is-last="{{($quizQuestion == $post->quizQuestions->last()) ? '1' : '0' }} ">
                    ...
                    @foreach(@$quizQuestion->quizAnswers as $key => $quizAnswer)
                        @if($quizQuestion->answer_format != 'text')
                            <div class="answers mb-3 {{ ($quizQuestion->answer_format == 'small_image') ? 'col-md-4': 'col-md-6' }} col-sm-12 answer-type-image">
                                ...
                                <div class="answer personality-quiz-answer " id="question_answer_{{ $quizAnswer->id }}"  data-post-id="{{ $post->id }}" data-question-id="{{ $quizQuestion->id }}" data-answer-id="{{ $quizAnswer->id }}" data-assigned-result-id="{{ $quizAnswer->result_id }}">
